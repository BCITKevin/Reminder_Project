...
export default class ReminderApp {
  ...
  private handleToggleCompletion(): void {
    ...
    const result = this.getUserChoice("reminder number to toggle", true);
    this._remindersHandler.toggleCompletion(Number(result) - 1);
  ...
  private handleModifyReminders(): void {
    ...
    Logger.logReminders(this._remindersHandler.reminders);
    const number = this.getUserChoice("reminder number", false);
    const description = this.getUserChoice("Description", false);
    this._remindersHandler.modifyReminder(Number(number) - 1, description);
    if(ReminderApp.checkUserToggleChoice()) {
      this._remindersHandler.toggleCompletion(Number(number) - 1);
    }
    Logger.log('\n  🏁   Reminder Modified');
  ...
  private handleAddReminder(): void {
    const newReminder = this.getUserChoice("reminder", false);
    const newTag = this.getUserChoice("tag", false);
    this._remindersHandler.addReminder(newReminder, newTag);
    Logger.log('\n  🏁  Reminder Added');
    /*
            Pseudocode:
