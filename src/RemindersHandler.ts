...
export default class RemindersHandler {
    ...
    public search(keyword: string): Reminder[] {
        let result = this.searchTags(keyword)
        if(result.length < 1) {
            let result = this.searchDescriptions(keyword)
            if(result.length < 1) {
                throw new Error("  ⚠️  You have no reminders")
            } else {
                return result;
            }
        } else {
    ...
    private searchDescriptions(keyword: string): Reminder[] {
        const list: Reminder[] = [];
        this._reminders.forEach(descriptions => {
            if(descriptions.description === keyword) {
